...
@router.get(
    "/{order_id}", response_model=List[ItemResponse], status_code=status.HTTP_200_OK
)
...
@router.put(
    "/{item_id}", response_model=ItemResponse, status_code=status.HTTP_201_CREATED
)
async def update_item(item_id: int, body: ItemModel, db: Session = Depends(get_db)):
